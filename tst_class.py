...
def get_result(opt, model):
    ...
    for idx, videoname in enumerate(video_list):
        ...
        labels = deepSBD(os.path.join(opt.root_dir, opt.test_subdir, videoname), opt.sample_duration, model,
                         spatial_transforms, opt.batch_size)
        _res = {'cut': [], 'gradual': []}
...
def tst(opt, model):
    ...
    # 先利用网络检测，将结果保存到results.json中，之后用于验证
